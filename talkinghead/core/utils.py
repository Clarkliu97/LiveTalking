...
def get_video_style_clip(
    ...
    if style_np is not None:
        pose_np = style_np
        face3d_exp = pose_np[:, 80:144]
    else:
        video_path = os.path.join(video_root_dir, video_name)
        if video_path[-3:] == "mat":
            mat_data = loadmat(video_path)
            face3d_all = mat_data["coeff"]
            face3d_all = face3d_all[30:]
            face3d_exp = face3d_all[:, 80:144]  # expression 3DMM range
        elif video_path[-3:] == "txt":
            face3d_exp = np.loadtxt(video_path)
        elif video_name[-3:] == 'npy':
            pose_np = np.load(video_path, allow_pickle=True)
            face3d_exp = pose_np[:, 80:144]
        else:
            raise ValueError("Invalid 3DMM file extension")
