...
class CoeffDetector(nn.Module):
    ...
    def forward(self, img, lm):
        ...
        # self.model.test()
        img = img[None].to(self.model.device)
        output_coeff = self.model.net_recon(img)
        pred_coeffs_dict = self.model.facemodel.split_coeff(output_coeff)

        if True:
            pred_coeff = {key: pred_coeffs_dict[key].detach().cpu().numpy() for key in pred_coeffs_dict}
            ...
            return {'coeff_3dmm': pred_coeff, 'crop_img': None}
            ...
            return {'coeff_3dmm':pred_coeff, 'crop_img': Image.fromarray((img.cpu().permute(1, 2, 0).numpy()*255).astype(np.uint8))}
...
if __name__ == "__main__":
    ...
    # opt.input_dir = "../talking_frames/face_crop"
    opt.input_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../talking_frames/face_crop"))
    pose_path = os.path.join(opt.input_dir,"full_pose.npy")
    opt.keypoint_dir = os.path.join(opt.input_dir,"keypoints")
    opt.output_dir = os.path.join(opt.input_dir,"coeff")
